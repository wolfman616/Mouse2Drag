...
global frames, fps_desired, fps_sleep, Playing, anus, HN, NID, max_index, fileinfo, fileinfo3, steam_iconhandle, EventScript
...
if DllCall("shell32\SHGetFileInfoW", "WStr",p00p , "UInt", 0, "Ptr", &fileinfo, "UInt", fisize := A_PtrSize + 688, "UInt", 0x100)
	
return,steam_iconhandle := NumGet(fileinfo, 0, "Ptr") 
}
xit: 
exitapp
...
;loop %max_index% { 
	; tt:= tt . " " . ( ringpiece := anus[a_index].process)
	;tt(ringpiece := anus[a_index].idx)
	;if (ringpiece = target_pos)
	;tt((anus[a_index].process) a_index) 

; tt(stt:=anus[target_pos+1].process)
; stt := ("Tray_" . (AN:= anus[target_pos+1].process))
stt := "TrayiconMenu" 
;tt(stt)
