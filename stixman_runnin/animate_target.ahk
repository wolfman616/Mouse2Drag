...
global frames,framestring,fps_desired,fps_sleep,Playing,filePath,OutFileName,OutDir,hund,uid,current,OutDir,OutExtension,OutNameNoExt
...
if frameindex {
	...
	{
	if !instr(framestring, a_loopfield . ",")
{	
	...
	loop,% (%ico_pfix%frames) {
		...
		if 	DllCall("shell32\SHGetFileInfoW", "WStr", varprefix%a_index%, "UInt", 0, "Ptr", &varprefix_i%a_index%, "UInt", fisize, "UInt", 0x100)
			(%ico_pfix%hbitmaps)[a_index]:= ({"HpGfx" : (NumGet(varprefix_i%a_index%, 0, "Ptr")) : "name" : varprefix%a_index%})
	}
...
TheBusiness(hWnd:="", u_i_D:="", framen:="") {
	...
	if  (%ico_pfix%ind  < (%ico_pfix%frames)) { 
		 %ico_pfix%ind +=  1
		if (current != ( %ico_pfix%frame_index)[%ico_pfix%ind] ) 
			current := ( %ico_pfix%frame_index)[%ico_pfix%ind]
	} else, current := ( %ico_pfix%ind := 1 )   ; start again

		if (%ico_pfix%hbitmaps[current].HpGfx && (%ico_pfix%hbitmaps[current].HpGfx != pprev)) {
